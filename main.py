...
'''
trading_signal_fast_fourier_transform_config = {"max_cache_len": 1100 if IS_LARGE_MEMORY else 400}
trading_signal_fast_fourier_transform_ = trading_signal.trading_signal_fast_fourier_transform(equity_data_, PCAf_, trading_signal_fast_fourier_transform_config)

portfolio_weights_feed_forward_network_config = {"PnL_evaluation_window_length": 24,
                                                "risk_aversion_factor": 2,
                                                "train_t_start": t_eval_start,
                                                "train_t_end": t_eval_end,
                                                "valid_t_start": t_eval_start,
                                                "valid_t_end": t_eval_end,
                                                "epoch_max": 20,
...
portfolio_weights_feed_forward_network_ = portfolio_weights.portfolio_weights_feed_forward_network(equity_data_, PCAf_, trading_signal_fast_fourier_transform_, portfolio_weights_feed_forward_network_config)
if PCA_TYPE == "name":
    portfolio_performance_.register_portfolio("PCA_name-FFT-FFN", portfolio_weights_feed_forward_network_, t_eval_start, t_eval_end)
if PCA_TYPE == "rank_theta_transform":
    portfolio_performance_.register_portfolio("PCA_rank_theta-FFT-FFN", portfolio_weights_feed_forward_network_, t_eval_start, t_eval_end)
if PCA_TYPE == "rank_hybrid_Atlas":
    portfolio_performance_.register_portfolio("PCA_rank_ha-FFT-FFN", portfolio_weights_feed_forward_network_, t_eval_start, t_eval_end)
'''
...
#t_eval_start = datetime.datetime(2011, 1, 1); t_eval_end = datetime.datetime(2022, 12, 15) # medium range
#t_eval_start = datetime.datetime(2015, 1, 1); t_eval_end = datetime.datetime(2022, 12, 15) # short range 1
#t_eval_start = datetime.datetime(2006, 1, 1); t_eval_end = datetime.datetime(2012, 12, 31) # short range 2
